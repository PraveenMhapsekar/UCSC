...
void *fn(void *str)
{
  ...
  printf("%s:Inside fn(): Thread '%d' Counter value: %d.\n",
	...
	while (i++<ITERS) {
   	...
   	if ((rc=pthread_mutex_lock(&m1))==0){
   		printf("%s:'%d' Acquired Mutex:Thread '%d'.\n",
				str,i,pthread_self());
   		counter++;
	 		sleep(5+i);			// sleep some arbitrary time to induce collisions
   		pthread_mutex_unlock(&m1);
   		printf("%s:'%d' Released Mutex:Thread '%d'.\n",
				str,i,pthread_self());
		}
		else
   		printf("%s:'%d' Not Acquired Mutex:Thread '%d'.\n",
				str,i,pthread_self());
