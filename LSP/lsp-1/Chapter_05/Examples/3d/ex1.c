// program renames a file
// it takes a backup before attempting rename.
...
int my_mvcmd(char *ofn, char *fn, char *erbuf) {
	...
	sigaddset(&set, SIGTERM); // term .. obviously won't work.
	sigaddset(&set, SIGALRM); // alarm
	sigaddset(&set, SIGTSTP); // terminal stp ^Z

	if (access(fn,F_OK) == 0) {
		sprintf(erbuf,"%s.%s: '%s' access failed:%s::",
 			ProgName,FuncName,ofn,"File has access issues");
		return -1;
	}
...
*/
	...
	if (access(ofn,F_OK) == 0) { // is file accessible? 
		...
		if ((st=my_cpcmd(tfn,fn,rbuf))!=0) { // now copy
			...
			unlink(fn);                    // underlying call for 'rm' cmd 
