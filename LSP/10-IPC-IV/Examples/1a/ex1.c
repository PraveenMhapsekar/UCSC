// world's first Ping program .. sourced from http://www.ping127001.com/pingpage/ping.text
...
zsignal(int sig, void *fn()) {

	struct sigaction oact, act;

	act.sa_handler=(void *)fn;
...
char *argv[];
{
	...
	zsignal(SIGINT, (void *)finish );
	zsignal(SIGALRM, (void *)catcher);
...
struct sockaddr_in *from;
{
	...
	if( (!(pingflags & QUIET)) && icp->icmp_type != ICMP_ECHOREPLY )  {
		printf("%d bytes from %s: icmp_type=%d (%s) icmp_code=%d\n",
		  cc, inet_ntoa(ntohl((int)from->sin_addr.s_addr)),
