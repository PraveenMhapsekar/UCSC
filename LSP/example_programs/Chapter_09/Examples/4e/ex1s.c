...
int read_and_send(int csd,int rfd,char *errbuf) {
	...
	if (recv(csd, str, sizeof(str),0) < 0) {
	...
	j=strlen(str)+1;
	if (send(csd,str,j,MSG_DONTWAIT|MSG_EOR)<j) {
		fprintf(stderr, "send() str Error: %s\n", strerror(errno));
		return -1;
	}
    ...
    //fprintf(stderr,"  \t\tData     \t: %s\n",ExampleData->data);
		sleep(1);
		if (send(csd,ExampleData,j,MSG_DONTWAIT|MSG_EOR)<j) {
			fprintf(stderr, "send() ExampleData Error: %s\n", strerror(errno));
			return -1;
		}
	}
