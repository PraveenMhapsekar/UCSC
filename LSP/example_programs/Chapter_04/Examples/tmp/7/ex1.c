...
int main(int argc, char **argv) {
  ...
  strcpy(dn, (argc>=2)? argv[1]: ".");
	strcpy(searchpat,"file_1_1.tst");
	// strcpy(searchpat,(argc>=3)? argv[2]:"");

	fprintf(stderr,"Searching for `%s` in `%s`.\n", searchpat,dn);
	
	// realpath() resolves . & .. and, other non-standard pathspecs 
	if (realpath(dn,rp)) {
...
int process_ls(char *dn) {
    ...
    if (lstat(dp->d_name, &statbuf) == -1) {
		...
		// if ((fnmatch(dp->d_name,searchpat))==0) {
		if ((strcmp(dp->d_name,searchpat))==0) {
			sprintf(buf,"%s/%s",dn,dp->d_name);	 
